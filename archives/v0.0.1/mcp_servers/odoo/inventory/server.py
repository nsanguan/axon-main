"""
mcp-ascp-inventory — Axon FastMCP server for Inventory (SSE transport).

This MCP server exposes Odoo inventory data to the Axon reasoning layer.
Swap or extend with adapters for other ERPs (SAP, Oracle) without changing agents.

Tools:
  axon_get_stock_quant      — query on-hand stock (stock.quant, read-only)
  axon_get_incoming_moves   — incoming stock moves by date/product (read-only)
  axon_get_outgoing_demand  — outgoing stock moves vs demand (read-only)
  axon_reserve_stock        — call action_assign on a picking (auto-chatter)
...
@mcp.tool()
def axon_get_stock_quant(input: GetStockQuantInput) -> list[dict]:
    """
...
@mcp.tool()
def axon_get_incoming_moves(input: GetIncomingMovesInput) -> list[dict]:
    """
...
@mcp.tool()
def axon_get_outgoing_demand(input: GetOutgoingDemandInput) -> list[dict]:
    """
...
@mcp.tool()
def axon_reserve_stock(input: ReserveStockInput) -> dict:
    """
