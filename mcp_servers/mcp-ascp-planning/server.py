"""
mcp-ascp-planning — FastMCP server (SSE transport).

Tools:
  ascp_get_ledger           — query era.ascp.pegging.ledger (read-only)
  ascp_update_allocation    — write allocated_qty + status (auto-chatter)
  ascp_create_exception     — set status=exception on a pegging record (auto-chatter)
  ascp_check_shortage       — compare open demand vs on-hand stock
  ascp_sync_demand_stream   — pull confirmed SOs → era.ascp.demand.stream (auto-chatter)
  ascp_get_supply_stream    — query era.ascp.supply.stream (read-only)
...
@mcp.tool()
def ascp_create_exception(input: CreateExceptionInput) -> dict:
    """
...
@mcp.tool()
def ascp_check_shortage(input: CheckShortageInput) -> list[dict]:
    """
...
@mcp.tool()
def ascp_sync_demand_stream(input: SyncDemandStreamInput) -> dict:
    """
...
@mcp.tool()
def ascp_get_supply_stream(input: GetSupplyStreamInput) -> list[dict]:
