"""
mcp-ascp-procurement — FastMCP server (SSE transport).

Tools:
  ascp_get_rfq_list         — list draft/sent purchase orders
  ascp_create_rfq           — create a new RFQ with lines (auto-chatter)
  ascp_confirm_po           — confirm a PO (auto-chatter)
  ascp_get_vendor_lead_time — get supplier lead times for a product
  ascp_analyse_rfq_impact   — impact analysis: cost/time delta vs. Odoo baseline
  ascp_post_comment         — post AI reasoning to any record's Chatter
  ascp_create_activity      — create a mail.activity for human approval (HITL)
  ascp_check_activity_done  — poll whether a mail.activity has been completed
"""
...
mcp = FastMCP("mcp-ascp-procurement")
...
@mcp.tool()
def ascp_get_rfq_list(input: GetRfqListInput) -> list[dict]:
...
@mcp.tool()
def ascp_create_rfq(input: CreateRfqInput) -> dict:
...
@mcp.tool()
def ascp_confirm_po(input: ConfirmPoInput) -> dict:
...
@mcp.tool()
def ascp_get_vendor_lead_time(input: GetVendorLeadTimeInput) -> list[dict]:
...
@mcp.tool()
def ascp_analyse_rfq_impact(input: AnalyseRfqImpactInput) -> list[dict]:
    """
...
@mcp.tool()
def ascp_analyse_po_for_approval(input: AnalysePoForApprovalInput) -> dict:
...
@mcp.tool()
def ascp_check_activity_done(input: CheckActivityDoneInput) -> dict:
