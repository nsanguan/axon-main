"""
CommunicationSkills — AI→Human bridge via Odoo Chatter and Activities.
...
- The target model MUST have mail.thread (all era.ascp.* models do).
...
class CommunicationSkills(BaseSkill):
    """
    Wraps Odoo mail.thread + mail.activity operations.
    Does NOT inherit post_ai_reasoning from BaseSkill to avoid circular calls —
    ...
    def post_ai_reasoning(
        ...
        The record's model MUST have mail.thread (all era.ascp.* models do).
    ...
    def create_activity(
        ...
        The workflow PAUSES here — LangGraph checks ascp_check_activity_done()
        before resuming.
